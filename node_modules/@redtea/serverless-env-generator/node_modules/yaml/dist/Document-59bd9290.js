...
class Document {
  ...
  createNode(value, {
    ...
    const ctx = {
      onAlias(source) {
        const alias = new resolveSeq.Alias(source);
    ...
    const node = resolveSeq.createNode(value, tag, ctx);
  ...
  set(key, value) {
    if (this.contents == null) {
      this.setSchema();
      this.contents = resolveSeq.collectionFromPath(this.schema, [key], value);
    } else {
      assertCollection(this.contents);
  ...
  setIn(path, value) {
    if (resolveSeq.isEmptyPath(path)) this.contents = value;else if (this.contents == null) {
      this.setSchema();
      this.contents = resolveSeq.collectionFromPath(this.schema, path, value);
    } else {
      assertCollection(this.contents);
  ...
  listNonDefaultTags() {
    return listTagNames(this.contents).filter(t => t.indexOf(PlainValue.defaultTagPrefix) !== 0);
  }
